                                  ...
                                  const TYPE(gsl_complex) twiddle4[])
{
  ...
  const ATOMIC sina = (ATOMIC) sin (2.0 * M_PI / 5.0);
  const ATOMIC sinb = (ATOMIC) sin (2.0 * M_PI / 10.0);
  ...
  for (k1 = 0; k1 < product_1; k1++)
    {
      ...
      const ATOMIC t1_real = ((ATOMIC) 2.0) * (z1_real + z2_real);
      const ATOMIC t2_real = ((ATOMIC) (2.0 * (sqrt (5.0) / 4.0))) * (z1_real - z2_real);
      const ATOMIC t3_real = z0_real - t1_real / (ATOMIC) 4.0;
      const ATOMIC t4_real = t2_real + t3_real;
      const ATOMIC t5_real = -t2_real + t3_real;
      const ATOMIC t6_imag = ((ATOMIC) 2.0) * (sina * z1_imag + sinb * z2_imag);
      const ATOMIC t7_imag = ((ATOMIC) 2.0) * (sinb * z1_imag - sina * z2_imag);
      ...
      for (k1 = 0; k1 < product_1; k1++)
        {
          ...
          /* t6 = (sqrt(5)/4)(t1 - t2) */
          const ATOMIC t6_real = ((ATOMIC) (sqrt (5.0) / 4.0)) * (t1_real - t2_real);
          const ATOMIC t6_imag = ((ATOMIC) (sqrt (5.0) / 4.0)) * (t1_imag - t2_imag);

          /* t7 = z0 - ((t5)/4) */
          const ATOMIC t7_real = z0_real - t5_real / (ATOMIC) 4.0;
          const ATOMIC t7_imag = z0_imag - t5_imag / (ATOMIC) 4.0;
  ...
  for (k1 = 0; k1 < product_1; k1++)
    {
      ...
      const ATOMIC t2_real = (t1_real / (ATOMIC) 4.0) - z2_real;
      const ATOMIC t3_real = ((ATOMIC) (sqrt (5.0) / 4.0)) * (z0_real - z1_real);
